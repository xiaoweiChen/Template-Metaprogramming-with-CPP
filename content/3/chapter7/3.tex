...
你会注意到knight和mage现在是没有任何基类的职业，都提供了step\_forth和step\_back成员函数，就像之前实现的CRTP模式。现在，movable\_unit类模板是从这些类中派生出来的，并定义了advance和retreat函数，并在循环中调用了step\_forth和step\_back。我们可以这样使用它们:
...
可以结合使用Mixins实现的静态多态，与使用接口和虚函数实现的动态多态。这里将通过一个关于战斗的游戏单位的例子来证明这一点。我们之前在讨论CRTP时举过一个例子，其中knight和mage职业都有一个叫做attack的成员函数。
...
接下来，将Mixins定义为能够独自或在团队中战斗的需求。这些类是模板，由它们的模板参数派生:
...
最后，我们需要定义knight和mage职业，这些本身就是战斗风格的Mixins。然而，为了能够在运行时多态地进行处理，我们从一个基本的game\_unit类中派生出它们，这个类包含一个叫做attack的纯虚方法:
...
knight和mage的attack成员函数使用了T::fight方法。细心的读者可能已经注意到，aggresive\_style和moderate\_style类，以及lone\_warrior和team\_warrior混合类都提供了这样的成员函数。所以，我们可以进行以下组合:
