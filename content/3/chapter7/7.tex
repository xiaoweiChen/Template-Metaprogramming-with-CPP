...
upgrade\_unit函数接受一个game\_unit对象，并对其应用upgrade\_defense和upgrade\_attack函数子。为此，它使用另一个名为apply\_functors的辅助函子，这是一个只有一个template参数的类模板。这个模板参数是一个类型列表。apply\_functors函子的可能实现如下所示:
