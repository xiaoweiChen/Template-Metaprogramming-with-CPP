...
\centering
	\begin{tabular}{|l|l|}
		...
		\textbf{描述} \\ \hline
		range &
		\begin{tabular}[c]{@{}l@{}}通过提供开始迭代器和结束前哨，定义类型R为范围的需求。\\ 迭代器和哨兵可以是不同的类型。\end{tabular} \\ \hline
		bgorrowed\_range &
		...
		common\_range &
		\begin{tabular}[c]{@{}l@{}}将类型R的要求定义为迭代器类型和前哨类型相同的范围。\end{tabular} \\ \hline
		view &
		...
		forward\_range &
		\begin{tabular}[c]{@{}l@{}}要求范围类型具有满足\\ forward\_iterator概念的迭代器类型。\end{tabular} \\ \hline
...
标准库为容器和数组定义了一系列访问函数。这包括std::begin和std::end代替成员函数begin和end, std::size代替成员函数size等，这些称为范围访问函数。类似地，范围库定义了一组范围访问函数。它们是为范围设计的，可以在<ranges>和<iterator>头文件和std::ranges命名空间中使用。下表列出了它们:
...
\centering
	\begin{tabular}{|l|l|l|}
		...
		data/cdata &
		data &
		\begin{tabular}[c]{@{}l@{}}分别返回指向连续范围和只读连续范围开头的指针。\end{tabular} \\ \hline
	\end{tabular}
\end{table}
...
\centering
	\begin{tabular}{|l|l|l|}
		...
		\begin{tabular}[c]{@{}l@{}}通过反复应用操作符\textgreater{}\textgreater\\ 生成一个元素序列的视图。\end{tabular} \\ \hline
	\end{tabular}
\end{table}
...
若想知道为什么empty\_view和single\_view有用，答案应该不难找到。这在模板代码中很有用，这些代码处理空范围或只有一个元素的范围是有效输入的范围。不希望一个函数模板多次重载来处理这些特殊情况；反之，可以传递一个empty\_view或single\_view范围。下面的代码片段展示了使用这些工厂的几个示例。这些代码段应该不言自明:
...
istream\_view范围工厂所做的是在istringstream对象上重复应用操作符>{}>，并在每次应用时产生一个值。不能指定分隔符,只适用于空格。若喜欢使用标准算法而不是手工循环，可以使用range::for\_each约束算法来产生相同的结果:
...
到目前为止，本章给出的示例包括filter, take, drop和reverse等视图。这些只是C++20中可用的一些标准视图，C++23中添加了更多，未来的标准版本中可能还会添加更多。下表列出了整个标准视图集:
...
目前为止，已经看到了使用过滤器、变换、反转和删除。为了完成本章的这一部分，我们通过一系列例子来演示表8.7中的视图的使用。以下所有示例中，我们将把rv视为std::ranges::views命名空间的别名:
