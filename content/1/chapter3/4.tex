...
若这个例子看起来有点复杂，那么一步步地分析它可能会帮助您更好地理解其工作原理。让我们从以下片段开始:
...
我们将使用cppinsights.io Web工具来检查，这个代码段中发生的模板实例化。首先要查看的是类模板元组，有一个主模板和几个特化:
...
tuple<int, double, char>结构包含一个int和一个tuple<double, char>；tuple<double, char>包含一个double和一个tuple<char>；tuple<char>又包含一个char值。最后一个类表示元组的递归定义的结束。在概念上可以用以下图形表示:
...
nth\_type结构体用作getter辅助类模板中的返回类型，其实例化如下所示:
...
最后，我们用来检索tuple元素值的get函数模板定义如下所示:
