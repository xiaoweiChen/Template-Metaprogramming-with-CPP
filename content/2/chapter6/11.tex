标准库提供了一组基本概念，可用于定义对函数模板、类模板、变量模板和别名模板的模板实参的需求，正如在本章中看到的那样。C++20中的标准概念分布在多个头文件和命名空间中。我们将在本节中介绍其中一些，你可以在\url{https://en.cppreference.com/}上找到所有的概念。

主要的概念集可以在<concepts>头文件和std命名空间中找到。这些概念中的大多数等价于一个或多个现有的类型特征。对于其中一些，它们的实现是定义良好的;对另一些，则是不明确的。它们可分为四类:核心语言概念、比较概念、对象概念和可调用概念。这组概念包含以下内容(但不仅如此):
...
\centering
	\begin{tabular}{|l|l|}
		...
		两种类型T和U具有共同引用类型。 \\ \hline
		common\_with &
		两种类型T和U具有共同类型，这两种类型都可以转换为共同类型。 \\ \hline
		...
		destructible &
		可以安全地销毁类型T的值(不需要从析构函数抛出任何异常)。 \\ \hline
		constructible\_from &
		可以用给定的参数网类型集构造类型为T的对象。 \\ \hline
...
其中一些概念是使用类型特征定义的，一些是其他概念或概念与类型特征的组合，还有一些至少部分具有未指定的实现。下面是一些例子:
...
\centering
	\begin{tabular}{|l|l|}
		...
		input\_iterator &
		类型是输入迭代器(支持读取、前增量和后增量)。 \\ \hline
		output\_iterator &
		类型是输出迭代器(支持写入、增量前和增量后)。 \\ \hline
		forward\_iterator &
		...
		\begin{tabular}[c]{@{}l@{}}作为bidirectional\_iterator类型的同时，也是随机访问迭代器(支持在常数时间\\ 内下标和前进)。\end{tabular} \\ \hline
		contiguous\_iterator &
		\begin{tabular}[c]{@{}l@{}}作为random\_access\_iterator类型同时，也是连续迭代器(元素存储在连续的内\\ 存位置)的要求。\end{tabular} \\ \hline
	\end{tabular}
\end{table}
...
它使用了几个概念(其中一些没有在这里列出)，以及一个requires表达式来确保某些表达式是格式良好的。
...
C++20中包含的几个主要特性之一(以及概念、模块和协程)是范围。范围ranges库定义了一系列类和函数，用于简化使用range的操作。其中有一组概念，在<ranges>头文件和std::ranges命名空间中定义。其中的一些概念:
